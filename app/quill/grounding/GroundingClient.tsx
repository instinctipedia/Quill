...
/* ---------------------------
   Rotating opener (client-only)
   --------------------------- */
function getOpenerLine() {
  const lines = [
    "Right. This bit.",
    "So… this is where we are.",
    "Nothing theoretical about this.",
    "What you’re dealing with.",
    "Let’s call it what it is.",
    "This is the part they don’t put in the manual.",
...
const [draftConcern, setDraftConcern] = React.useState(concern);
  ...
  // legislation origin selector
  const [origin, setOrigin] = React.useState<HSEOrigin>("international");

  // opener (hydration-safe)
  const [opener, setOpener] = React.useState<string | null>(null);

  React.useEffect(() => {
    setOpener(getOpenerLine());
  }, []);
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!res.ok || !json) {
          setData({ status: "error", error: "Failed to load grounding data." });
        } else {
          setData(json);
        }
      } catch {
  ...
  const visibleHSE =
    data?.hse?.filter(
      (s) => s.origin === "guidance" || s.origin === origin
    ) ?? [];
 ...
 {/* Concern input (advisory entry point) */}
  <div
    style={{
      border: "1px solid #ddd",
      borderRadius: 14,
        ...
        <div style={{ marginTop: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginTop: "1rem" }}>
            <button
              type="button"
              onClick={() =>
                setParam("more_cases", moreCases ? undefined : "1")
              }
            >
        ...
        <div style={{ marginTop: "1.5rem" }}>
          ...
          >
            ...
            <select
              value={origin}
              onChange={(e) =>
                setOrigin(e.target.value as HSEOrigin)
              }
            >
          ...
          <div style={{ display: "flex", gap: "0.75rem", marginTop: "1rem" }}>
            <button
              type="button"
              onClick={() =>
                setParam("explain_law", explainLaw ? undefined : "1")
              }
            >
